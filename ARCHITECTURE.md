...
│   ├── Risk badge (LOW/MEDIUM/HIGH with score)
...
└── Footer (analysis metadata: started, completed, tool version)

Dashboard (/dashboard)
├── Header
├── Filters (search by app name, filter by classification)
├── Analyses table
│   ├── Columns: Date | App | Package | Hash (truncated) | Classification | Actions
...
### 11.1 Forensic-grade reporting (Phase 6 + `modules/forensic.py`)

**What gets added beyond a normal PDF report:**

- **APK integrity proof.** The SHA-256 hash of the uploaded APK is computed at upload time, displayed on the result page, and embedded in the PDF cover page. Re-hashing the same APK later must produce the same hash — proving the analyzed artifact has not been tampered with.
- **Analysis metadata block** on the PDF cover: analyst name field (defaults to "Anonymous Analyst"), tool version, started/completed timestamps, hash, dynamic/SBP/educational flags.
- **Audit log appendix** at the end of the PDF: chronological list of every state change from `audit_log` table (analysis_started → phase_2_completed → ... → analysis_completed) with timestamps.
- **Tool version statement** — important for forensic reproducibility ("this report was generated by SecureAPK v1.0.0 on YYYY-MM-DD").
